...
bot.on('callback_query', query => {
  switch (query.data) {
    ...
    case 'login':
      ...
      axios.get(`https://back.babirusa.skifry.ru/log_tg?tg_id=${query.from.id}`)
      .then(res => {
    ...
    case 'accept_reg':
      if (current_input == 4) {
        ...
        axios.post('https://back.babirusa.skifry.ru/reg_tg', user)
        .catch(err => {
          console.log(err.code);
          bot.sendMessage(query.message.chat.id, 'Что-то пошло не так, попробуйте заново! 😢', {
            reply_markup: {
    ...
    case 'code':
      ...
      bot.onText(/[0-9]/, msg => {
        if (msg.text.length === 6) {
          ...
          axios.post('https://back.babirusa.skifry.ru/code_check', {
            code: msg.text,
            tg_id: msg.from.id
          }).then(res => {
            if (res.data === 'ok') {
